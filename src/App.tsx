...
import { DissonanceKnot } from './components/game/enemies/DissonanceKnot'
import { HarmonicLock } from './components/game/puzzles/HarmonicLock'
import { BossEntity } from './components/game/enemies/BossEntity'
import { ResonanceHarvest } from './components/game/puzzles/ResonanceHarvest'
...
const Scene = ({ viewMode, setViewMode, settings, setSettings, mathSettings, setPunchDataState }: { 
    ...
    inputSources.forEach((inputSource, index) => {
      ...
      if (t - lastButtonPress.current > 0.5) {
        if (gamepad?.['a-button']?.pressed || gamepad?.['x-button']?.pressed) {
          ...
          setViewMode(modes[nextIndex] as 'fluid' | 'math' | 'hybrid')
  ...
  const { gameState, chamberType, nextChamber, addScore } = useGameStore()
  ...
  return (
    <>
      ...
      <group onPointerDown={onPointerDown}>
        ...
        {/* Game Entities injected here */}
        {gameState === 'playing' && chamberType === 'puzzle' && (
          <HarmonicLock 
             punchData={punchData} 
             onUnlock={() => {
                addScore(500);
        ...
        {gameState === 'playing' && chamberType === 'combat' && (
          <DissonanceKnot 
        ...
        {gameState === 'playing' && chamberType === 'harvest' && (
          <ResonanceHarvest 
             mathSettings={mathSettings}
             activeBeams={activeBeams}
             onComplete={() => {
        ...
        {gameState === 'playing' && chamberType === 'boss' && (
          <BossEntity 
             mathSettings={mathSettings} 
             punchData={punchData} 
             activeBeams={activeBeams} 
...
export default function App() {
  ...
  return (
    <div className="w-screen h-screen bg-[#000000] overflow-hidden relative font-sans text-white">
      <HUD />
      <MainMenu onStartVR={handleEnterVR} vrSupported={xrSupported} />
      ...
      {/* Math Editor Panel */}
      <AnimatePresence>
        {gameState === 'playing' && (viewMode === 'math' || viewMode === 'hybrid') && (
          <motion.div 
          ...
          <div className="flex gap-4">
            {[
              ...
              <button 
                key={mode.id}
                onClick={() => {
                  setViewMode(mode.id as 'fluid' | 'math' | 'hybrid')
                  playSound('ui', settings.soundVolume)
                }}
